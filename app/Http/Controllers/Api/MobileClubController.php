...
class MobileClubController extends Controller
{
    ...
    public function preview(Request $request, int $tenantId)
    {
        ...
        return response()->json($this->buildClubPayload($tenantId, (int)$client->id));
    }
    ...
    public function profile(Request $request)
    {
        ...
        return response()->json($this->buildClubPayload($tenantId, $clientId));
    }
    ...
    public function reviews(Request $request)
    {
        ...
        $data = $rows->map(function (ClubReview $r) {
            $a = (int)($r->atmosphere_rating ?? $r->rating);
            $c = (int)($r->cleanliness_rating ?? $r->rating);
            $t = (int)($r->technical_rating ?? $r->rating);
            $p = (int)($r->peripherals_rating ?? $r->rating);
            return [
                'id' => (int)$r->id,
                'client_id' => (int)$r->client_id,
                'client_login' => (string)($r->client?->login ?? ('#' . $r->client_id)),
                'rating' => (int)$r->rating,
                'atmosphere_rating' => $a,
                'cleanliness_rating' => $c,
                'technical_rating' => $t,
                'peripherals_rating' => $p,
        ...
        return response()->json([
            ...
            'mine' => $mine ? [
                ...
                'atmosphere_rating' => (int)($mine->atmosphere_rating ?? $mine->rating),
                'cleanliness_rating' => (int)($mine->cleanliness_rating ?? $mine->rating),
                'technical_rating' => (int)($mine->technical_rating ?? $mine->rating),
                'peripherals_rating' => (int)($mine->peripherals_rating ?? $mine->rating),
                'comment' => (string)($mine->comment ?? ''),
                'updated_at' => optional($mine->updated_at)?->toIso8601String(),
                'created_at' => optional($mine->created_at)?->toIso8601String(),
            ] : null,
    ...
    {
        if (!Schema::hasTable('club_reviews')) {
            return response()->json(['message' => 'Reviews are not ready. Run migration first.'], 422);
        }

        $hasDims =
            Schema::hasColumn('club_reviews', 'atmosphere_rating') &&
            Schema::hasColumn('club_reviews', 'cleanliness_rating') &&
            Schema::hasColumn('club_reviews', 'technical_rating') &&
            Schema::hasColumn('club_reviews', 'peripherals_rating');
        if (!$hasDims) {
            return response()->json(['message' => 'Review dimensions are not ready. Run migration first.'], 422);
        }
        ...
        $fallback = isset($data['rating']) ? (int)$data['rating'] : null;
        $a = isset($data['atmosphere_rating']) ? (int)$data['atmosphere_rating'] : $fallback;
        $c = isset($data['cleanliness_rating']) ? (int)$data['cleanliness_rating'] : $fallback;
        $t = isset($data['technical_rating']) ? (int)$data['technical_rating'] : $fallback;
        $p = isset($data['peripherals_rating']) ? (int)$data['peripherals_rating'] : $fallback;
    ...
    private function buildClubPayload(int $tenantId, int $clientId): array
    {
        ...
        $latestMetricsByPc = [];
        if (!empty($pcIds) && Schema::hasTable('pc_heartbeats')) {
        ...
        foreach ($knownZones as $zone) {
            ...
            if (!isset($zoneStats[$nameKey])) {
                ...
                // Duplicate zone names are merged in mobile preview.
        ...
        usort($zones, fn($a, $b) => strcasecmp((string)$a['name'], (string)$b['name']));
        usort($pcItems, function ($a, $b) {
            $zCmp = strcasecmp((string)($a['zone_name'] ?? ''), (string)($b['zone_name'] ?? ''));
    ...
    private function buildClubInsights(
        int $tenantId,
        array $zones,
        array $pcItems,
        int $totalPcs,
        ...
        usort($heatmap, fn($a, $b) => strcasecmp((string)$a['zone_name'], (string)$b['zone_name']));

        $bestHours = collect(range(0, 23))
            ->map(function (int $hour) use ($hourTotals) {
                return [
                    'hour' => $hour,
    ...
    private function buildLiveQueue(int $tenantId, array $zones, array $pcMetaById): array
    {
        ...
        foreach ($zoneRows as $zoneKey => $row) {
            ...
            $eta = $freeNow > 0 ? 0 : (int)($etaByZone[$zoneKey] ?? max(5, (int)round(($zoneAvgDuration[$zoneKey] ?? 120) / 2)));
            $row['eta_min'] = $eta;
        ...
        usort($queue, function ($a, $b) {
            $freeCmp = (int)$b['free_now'] <=> (int)$a['free_now'];
            if ($freeCmp !== 0) {
                return $freeCmp;
            }
    ...
    private function buildSmartBundleInsights(int $tenantId, array $zones): array
    {
        ...
        foreach ($packages as $p) {
            ...
            $zoneNorm = 'name:' . $this->normalizeZoneName((string)$p->zone);
        ...
        foreach ($plans as $plan) {
            ...
            $effectivePerHour = (int)round($price / ($days * 2)); // 2h/day assumption
            $savePerHour = max(0, $zoneRate - $effectivePerHour);
            if ($savePerHour <= 0) {
                continue;
            }
        ...
        usort($items, function ($a, $b) {
            $s = (int)($b['save_percent'] ?? 0) <=> (int)($a['save_percent'] ?? 0);
            if ($s !== 0) {
                return $s;
            }
    ...
    private function settingString($settings, string $key): ?string
    {
        ...
        if (is_array($value)) {
            foreach (['url', 'path', 'value', 'text', 'logo'] as $variant) {
                $v = isset($value[$variant]) ? trim((string)$value[$variant]) : '';
                if ($v !== '') {
                    return $v;
                }
            }
