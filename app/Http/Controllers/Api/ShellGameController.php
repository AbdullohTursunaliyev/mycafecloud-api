...
class ShellGameController extends Controller
{
    public function index(Request $request)
    {
        ...
        $pcCode = (string)$request->query('pc_code', '');
        ...
        $clientId = (int)$request->attributes->get('client_id', 0);
    ...
    public function adminIndex(Request $request)
    {
        ...
        $pcId = (int)$request->query('pc_id', 0);
        $pc = null;
        if ($pcId > 0) {
            $pc = Pc::query()
                ->where('tenant_id', $tenantId)
    ...
    {
        $tenantId = (int)$request->user()->tenant_id;
        $row = ShellGame::query()->where('tenant_id', $tenantId)->findOrFail($id);
        $data = $request->validate($this->rules($tenantId, $row->id));
        $row->fill($data)->save();
    ...
    public function toggle(Request $request, int $id)
    {
        ...
        return response()->json(['data' => ['id' => $row->id, 'is_active' => (bool)$row->is_active]]);
    }

    public function setPcState(Request $request, int $pcId, int $gameId)
    {
        $tenantId = (int)$request->user()->tenant_id;
        ...
        $row = PcShellGame::query()->updateOrCreate(
            [
                'tenant_id' => $tenantId,
                'pc_id' => $pc->id,
            ...
            [
                'is_installed' => (bool)$data['is_installed'],
    ...
    {
        $exists = ShellGame::query()->where('tenant_id', $tenantId)->exists();
        if ($exists) {
            return;
        }
